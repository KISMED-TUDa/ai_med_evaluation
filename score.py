...
Computes scores for the course (F1 and Multi Label)
...
def score(test_dir='../test/'):
    ...
    if not os.path.exists("PREDICTIONS.csv"):
        sys.exit("Predictions not found!")  

    if  not os.path.exists(os.path.join(test_dir, "REFERENCE.csv")):
        sys.exit("Ground truth not found!")  

    df_pred = pd.read_csv("PREDICTIONS.csv", header=None)   # classification
    df_gt = pd.read_csv(os.path.join(test_dir,"REFERENCE.csv"), header=None)  # truth

    N_files = df_gt.shape[0]    # number of datapoints

    ## init variables for F1 Score
    TP = 0  # true positives
    TN = 0  # true negatives
    FP = 0  # false positives
    FN = 0  # false negatives

    ## init variables for multi label score
    ...
    for i in range(N_files):
        ...
        if not pred_indx.size:
            print("Prediction for " + gt_name + " missing, assume \"normal\" .")
            pred_class = "N"
        else:
